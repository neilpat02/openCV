    ...
    def get_teams_sorted_by_timestamp(self):
        ...
        for user in users:
            if user["lastUploadToBotTimestamp"] is not None:
                teams.append(
                    (user["teamName"], user["lastUploadToBotTimestamp"].isoformat())
                )
        return teams

    def update_hardware_score(self, team_name, score):
        self.users.update_one(
            {"teamName": team_name}, {"$set": {"hardwareScore": score}}
        )
...
class MainWindow:
    ...
    def init_ui(self):
        ...
        self.label = Label(
            self.root,
            text="Teams are being processed automatically based on upload timestamp:",
            font=label_font,
        )
        self.label.pack(pady=10)

        self.roi_button = Button(
            self.root, text="Select ROI", font=button_font, command=self.select_roi
        )
        self.roi_button.pack(pady=10)

        self.refresh_button = Button(
            self.root, text="Refresh Teams", font=button_font, command=self.load_teams
        )
    ...
    def update_start_timer_button_text(self):
        ...
        if self.selected_team_name:
            self.start_timer_button.config(
                text=f"Start Timer for {self.selected_team_name}"
            )
        else:
    ...
    def start_processing(self):
        if self.teams and not self.timer_started:
            ...
            messagebox.showinfo(
                "Team Selected", f"Selected Team: {self.selected_team_name}"
            )
    ...
    def display_score_and_proceed(self, exploration_score, final_time_score):
        ...
        messagebox.showinfo(
            f"Scores for {team_name}",
            f"Exploration Score: {exploration_score}\nTime Score: {final_time_score}\nFinal Score: {final_score}",
        )
    ...
    def halt_until_detected_moves(self):
        ...
        while self.timer_started:
            ...
            if not last_detections:
                # assign the starting detections if it's the first frame
                ...
                # assign starting detections if we suddenly detect a new bot and perform check again.
                starting_detections = detections
                continue
            ...
            for detection in detections:
                ...
                start_detection = min(
                    starting_detections, key=lambda x: dist(x, detection)
                )
    ...
    def process_detections(self, team_name: str, roi=None, total_time_seconds=None):
        ...
        while self.timer_started:
            ...
            self.update_frame(
                frame, roi, cell_size_x, cell_size_y, visited_cells, detections
            )
            exploration_score = min(len(visited_cells) * 2, self.max_cells * 2)

            if elapsed_time > 60:
                if (
                    current_time - last_score_reduction_time
                    >= MainWindow.score_reduction_interval
                    and final_time_score > 0
                ):
                    final_time_score = max(
                        final_time_score - MainWindow.score_reduction_rate, 0
                    )
    ...
    def update_frame(
        self, frame, roi, cell_size_x, cell_size_y, visited_cells, detections
    ):
        for i in range(9):
            cv2.line(
                frame,
                (roi[0] + i * cell_size_x, roi[1]),
                (roi[0] + i * cell_size_x, roi[1] + roi[3]),
                (68, 222, 253),
                2,
            )
            cv2.line(
                frame,
                (roi[0], roi[1] + i * cell_size_y),
                (roi[0] + roi[2], roi[1] + i * cell_size_y),
                (68, 222, 253),
                2,
            )
